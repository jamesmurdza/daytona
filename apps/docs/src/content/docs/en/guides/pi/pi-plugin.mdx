...
The sandbox pauses after a short idle period (its filesystem is preserved) and is transparently restarted on the next tool call or when you resume the session.
