...
The rest of this section shows the pieces that implement that flow.

#### Tool registration

Source: [`src/tools.ts`](https://github.com/daytonaio/daytona/blob/main/libs/pi-plugin/src/tools.ts)

Each built-in tool is wrapped so that, when a sandbox is active, it runs against it through the [Daytona SDK](https://www.npmjs.com/package/@daytona/sdk). The operation-backed tools share one helper:
...
`requireSandbox()` makes sure tools never silently run on your host: with `--daytona` set but no sandbox available, the call fails instead of falling back to local execution.
