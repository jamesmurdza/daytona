...
1. **Tool registration** — at startup the extension swaps Pi's built-in tools for sandbox-backed ones.
2. **`session_start`** — create or reattach this session's sandbox and clone its `pi/<id>` branch into it.
3. **`before_agent_start`** — rewrite the system prompt so the agent targets the sandbox.
4. **Each tool call** — runs inside the sandbox, never on your host.
5. **`agent_end`** — push the agent's new commits to the branch.
6. **Idle / resume** — the sandbox pauses when idle and is reattached when you resume the session.
7. **Session deleted** — the sandbox is reaped on the next launch or exit.
