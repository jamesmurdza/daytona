...
try {
	...
	check(err && err.name === "SandboxUnavailableError", "throws SandboxUnavailableError (not a raw Docker error)", err && err.name);
	check(err && !/no such container|inspect sandbox/i.test(err.message), "error message is human-friendly", err && err.message?.slice(0, 80));
